...
import { decompress } from './SHPCompress';
import path from 'path';
import { SHPLoader } from './three';
...
export class SHPParser {
  ...
  /**
   * Author : lolorisotto
   * Convert a NonSharedBuffer into a ArrayBuffer
   * @param buffer 
   * @returns 
   */
  static toArrayBuffer(buffer:NonSharedBuffer) {
   ...
   * Author Lolorisotto
   * 
   * Load shp to JSON file
   * @param src 
   * @param output 
   * @param spherize 
   * @returns 
   */
  // static async shpToJSON(src:string, output:string, spherize:boolean = false): Promise<boolean> {

  //   // Load shp as ShapeFileData
  //   const shapeFileData = await SHPParser.load(src)

  //   // Load ShapeFileData into 3D Object
  //   const loader = new SHPLoader();
  //   const landModel = loader.createModel(shapeFileData, { spherize: spherize });

  //   // to JSON
  //   const landModelJSON = landModel.toJSON()

  //   // Write file
  //   const jsonPathname =  path.join(process.cwd(), output);
  //   writeFileSync(jsonPathname, JSON.stringify(landModelJSON))
  //   return false
  // }
